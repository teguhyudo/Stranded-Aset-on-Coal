...
This appendix provides full derivations and parameter specifications for the three models presented in Sections~\ref{sec:model1}--\ref{sec:model3}.
...
\begin{equation}
\label{eq:npv_full}
\text{NPV}_{i,s} = \sum_{t=1}^{T_i} \frac{\left(P_{s,t} - C_i\right) \cdot Q_i \cdot (1 - \tau)}{(1 + r_i)^t}
\end{equation}
...
\begin{itemize}
    ...
    \item $Q_i$ is the annual production volume for firm $i$ (million tonnes), also held constant;
    \item $\tau = 0.22$ is the Indonesian corporate income tax rate;
    \item $r_i$ is the firm-specific weighted average cost of capital (WACC); and
